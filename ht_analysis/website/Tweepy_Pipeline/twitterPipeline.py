...
def acquire_tweets(hashtag: str, upper_limit: int = None) -> pd.DataFrame:
    ...
    if hashtag is not None:
        # Ensure correct format of given hashtag
    ...
    tweets = tweepy.Cursor(tweets_access_api.search_tweets, q=tweet_hashtag,
                           count=200, tweet_mode='extended', lang='en').items(number_of_hashtag_tweets)
    
    # Creating the dataframe with pandas
    columns = ['Tweets', 'Time',"Location"] 
    data = []
